            ...
            {data.data.map((i: any) => (
              <div key={i.id} className="px-4 py-3 flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="font-semibold text-slate-800 truncate">{titleCase(i.category)}</p>
                  <p className="text-xs text-slate-400 truncate">{i.resident ?? "—"} · {formatDate(i.date)}</p>
                </div>
                <span className="font-bold text-emerald-600 shrink-0">{formatPKR(i.amount)}</span>
              </div>
            ))}
          </div>
