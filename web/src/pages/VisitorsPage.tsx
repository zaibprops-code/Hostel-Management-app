            ...
            {data.map((v) => (
              <div key={v.id} className="px-4 py-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {v.departureTime ? <span className="text-xs text-slate-400">Left</span> : <Badge color="green">Inside</Badge>}
                </div>
                <p className="text-xs text-slate-400 truncate mt-0.5">{v.purpose ?? "Visit"} · {formatDateTime(v.arrivalTime)}</p>
                {can("visitors.manage") && !v.departureTime && <button onClick={() => checkout(v.id)} className="text-brand-600 text-sm font-medium mt-2">Check out</button>}
              </div>
            ))}
          </div>
